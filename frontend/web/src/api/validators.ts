...
export const publicFolderShareAccess: Validator<PublicFolderShareAccess> = (value, path) => {
  const item = object(value, path)
  return {
    id: string(prop(item, 'id', path), `${path}.id`),
...
export const folder: Validator<Folder> = (value, path) => {
  ...
  return {
    ...
    share_token: nullableString(prop(item, 'share_token', path), `${path}.share_token`),
    share_starts_at: nullableString(optionalProp(item, 'share_starts_at') ?? null, `${path}.share_starts_at`),
    share_expires_at: nullableString(optionalProp(item, 'share_expires_at') ?? null, `${path}.share_expires_at`),
    share_download_limit: nullableNumber(optionalProp(item, 'share_download_limit') ?? null, `${path}.share_download_limit`),
    share_download_count: number(optionalProp(item, 'share_download_count') ?? 0, `${path}.share_download_count`),
    share_one_time: boolean(optionalProp(item, 'share_one_time') ?? false, `${path}.share_one_time`),
    share_password_enabled: boolean(optionalProp(item, 'share_password_enabled') ?? false, `${path}.share_password_enabled`),
    share_recipient_email: nullableString(optionalProp(item, 'share_recipient_email') ?? null, `${path}.share_recipient_email`),
    created_at: string(prop(item, 'created_at', path), `${path}.created_at`),
    updated_at: string(prop(item, 'updated_at', path), `${path}.updated_at`),
    is_deleted: boolean(prop(item, 'is_deleted', path), `${path}.is_deleted`),
    deleted_at: nullableString(prop(item, 'deleted_at', path), `${path}.deleted_at`),
...
export const publicFolderShareAccessEvents = arrayOf(publicFolderShareAccess)
